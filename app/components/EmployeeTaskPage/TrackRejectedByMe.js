...
import { fetchRejected } from "@/app/controllers/approvalController";
import Cookies from "js-cookie";
import ViewModal from "./ViewModal";
import { toast } from "react-toastify";

const TrackRejectedByMe = () => {
  ...
  const fetchRequestData = useCallback(async (approverId, page = 1) => {
    ...
    try {
      const response = await fetchRejected(approverId, page);
  ...
  const refreshData = () => {
    fetchRequestData(approverId?.pagination?.currentPage || 1);
  };
...
export default TrackRejectedByMe;
