    ...
    if (isOpen) {
      const fetchForm = async () => {
        try {
          ...
          // Only set state when all data is ready
          setFormSchema(formSchema);
          setFormType(formType);
          setFormData(initialData);
          setEmployeeDetails(employeeData);

          console.log("employeeDetails: ", employeeData); // Log employee data here
        } catch (err) {
    ...
    if (field.type === "radio" || field.type === "checkbox") {
      ...
      return (
        ...
        : formData[field.name] || ""
            ...
            onChange={handleChange}
            required={field.required}
            disabled={field.disabled} // Dynamically disabling based on schema
