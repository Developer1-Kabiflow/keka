...
export const fetchFormSchema = async (formId) => {
  try {
    const response = await axios.get(`${BASE_URL}/template/fetchForm/${formId}`);
...
//Fetch oneFormData from backend
...
export const submitFormData = async (formId, formData) => {
  try {
    const response = await axios.post(`${BASE_URL}/request/addRequest/${formId}`, formData);
    return response.data;
  } catch (error) {
